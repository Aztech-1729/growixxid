"""Async client for TigerSMS (SMS-Activate style API).

Docs: https://tiger-sms.com/api
Base: https://api.tiger-sms.com/stubs/handler_api.php
Auth: ``api_key`` query param. Responses are plain TEXT (SMS-Activate dialect):
  getBalance      -> ACCESS_BALANCE:0
  getPrices       -> JSON {country_id: {service: {cost, count}}}
  getCountries    -> JSON [{id, rus, eng, chn, visible, retry}]
  getNumber       -> ACCESS_NUMBER:<id>:<number>   | NO_BALANCE / NO_NUMBERS
  getStatus       -> STATUS_WAIT_CODE | STATUS_OK:<code> | STATUS_CANCEL | NO_ACTIVATION
  setStatus       -> status=1 (confirm received) / status=8 (cancel)

TigerSMS is the cheapest India TG source found (~$0.12 ~= ₹10, 368k stock).
India numeric country id = 6. Services: tg, wa.
"""
import asyncio
import json

import httpx

from config import config


class TigerSMSError(Exception):
    """Raised on ANY provider failure (business or network)."""


class TigerSMS:
    BASE = "https://api.tiger-sms.com/stubs/handler_api.php"

    def __init__(self, api_key: str, timeout: float = 20.0, retries: int = 3):
        self.api_key = api_key
        self.timeout = timeout
        self.retries = retries
        self._client = httpx.AsyncClient(timeout=timeout)

    async def close(self) -> None:
        await self._client.aclose()

    async def _req(self, action: str, **params) -> str:
        if not self.api_key:
            raise TigerSMSError("TigerSMS API key is not configured.")
        params = {"api_key": self.api_key, "action": action, **params}
        for attempt in range(self.retries):
            try:
                r = await self._client.get(self.BASE, params=params)
                return (await r.aread()).decode("utf-8", "replace").strip()
            except httpx.HTTPError:
                if attempt < self.retries - 1:
                    await asyncio.sleep(1.0 * (attempt + 1))
                    continue
                break
        raise TigerSMSError("TigerSMS is temporarily unreachable. Please try again.")

    async def balance(self) -> float:
        t = await self._req("getBalance")
        if t.startswith("ACCESS_BALANCE:"):
            return float(t.split(":", 1)[1])
        raise TigerSMSError(t)

    async def countries(self, service: str) -> list:
        t = await self._req("getCountries", service=service)
        try:
            return json.loads(t)
        except Exception:
            raise TigerSMSError(t)

    async def prices(self, service: str, country: str = None) -> dict:
        p = {}
        if country:
            p["country"] = country
        t = await self._req("getPrices", service=service, **p)
        try:
            return json.loads(t)
        except Exception:
            raise TigerSMSError(t)

    async def get_number(self, service: str, country: str) -> tuple:
        t = await self._req("getNumber", service=service, country=country)
        if t.startswith("ACCESS_NUMBER:"):
            _, aid, number = t.split(":", 2)
            return aid, number
        raise TigerSMSError(t)

    async def get_status(self, activation_id: str) -> str:
        return await self._req("getStatus", id=activation_id)

    async def set_status(self, activation_id: str, status: int) -> str:
        return await self._req("setStatus", id=activation_id, status=status)


tigersms = TigerSMS(config.TIGERSMS_API_KEY)
